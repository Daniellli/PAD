...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i_iter_val, (_, val_vox_label, val_grid, val_pt_labs, val_pt_fea, idx) in enumerate(
            ...
            coor_ori, output_normal_dummy, combined_output_normal_dummy = my_model.forward_dummy_final(
                val_pt_fea_ten, val_grid_ten, val_batch_size, args.dummynumber, PROCESS_BLOCK_3=False, PROCESS_BLOCK_4=False)
            predict_labels = output_normal_dummy[:,:-1,...]

            # aux_loss = loss_fun(aux_outputs, point_label_tensor)
            loss = lovasz_softmax(torch.nn.functional.softmax(predict_labels).detach(), val_label_tensor,
                                  ignore=0) + loss_func(predict_labels.detach(), val_label_tensor)
            uncertainty_scores_logits = combined_output_normal_dummy[:,-1,...]
            uncertainty_scores_logits = uncertainty_scores_logits.cpu().detach().numpy()

            softmax_layer = torch.nn.Softmax(dim=1)
            uncertainty_scores_softmax = softmax_layer(combined_output_normal_dummy)[:,-1,...]
            uncertainty_scores_softmax = uncertainty_scores_softmax.cpu().detach().numpy()

            # TODO: decide when to run this block of codes
            if True:
                ...
                logits_for_loss_computing = torch.hstack(
                    [combined_output_normal_dummy[:, 1:5], combined_output_normal_dummy[:, 6:-1]])
            ...
            point_predict.tofile('../../semantic_kitti/predictions/sequences/08/predictions_2dummy_1_01_final_cross_latest/' + idx_s + '.label')
