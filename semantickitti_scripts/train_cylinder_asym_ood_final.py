...
def main(args):
    ...
    # experiment_output_dir = join(os.getcwd(),'..','runs', time.strftime("%Y-%m-%d-%H:%M:%s",time.gmtime(time.time())), args.experiment_name)
    ...
    while epoch < train_hypers['max_num_epochs']:
        ...
        if SAVE_MODIFIED_DATA and epoch == 1:
            break
        # Validate
        if True and not SAVE_MODIFIED_DATA:
            my_model.eval()
            hist_list = []
            val_loss_list = []
            with torch.no_grad():
