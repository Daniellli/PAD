...
class SemKITTI_sk_panop(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if SAVE_MODIFIED_DATA:
            data_tuple = (raw_data[:, :3], semantic_data, instance_data)
        else:
...
class SemKITTI_sk_panop_incre(data.Dataset):
    def __init__(self, data_path, imageset='train',
        ...
        self.pred_names = []
        pred_paths = "../../semantic_kitti/predictions/sequences/08/predictions_base_train"
...
class SemKITTI_nusc_panop(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        points_label = np.load(lidarseg_labels_filename, allow_pickle=True)['data'].reshape([-1, 1])
