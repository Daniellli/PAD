...
class cylinder_dataset_nuscenes(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=0, return_test=False,
                 ...
                 min_rad=-np.pi / 4, max_rad=np.pi / 4, ds_sample=False, incre=None, SHAPENET_ANOMALY=False):
        'Initialization'
...
class cylinder_dataset_nuscenes_panop(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=0, return_test=False,
                 fixed_volume_space=False, max_volume_space=[50, np.pi, 3], min_volume_space=[0, -np.pi, -5],
                 scale_aug=False, transform_aug=False, trans_std=[0.1, 0.1, 0.1],
                 min_rad=-np.pi / 4, max_rad=np.pi / 4, ds_sample=False, incre=None, SHAPENET_ANOMALY=False,shapenet_path=None,debug=False):
        'Initialization'
        self.point_cloud_dataset = in_dataset
        self.debug =debug
        self.grid_size = np.asarray(grid_size)
        ...
        self.noise_rotation = np.random.uniform(min_rad, max_rad)
        self.SHAPENET_ANOMALY = SHAPENET_ANOMALY
    ...
    def __getitem__(self, index):
        ...
        #!======================================================
        # write_ply_color(xyz,labels.squeeze(),'logs/debug/scene1.ply')
        #!======================================================
        ...
        if self.SHAPENET_ANOMALY:
            ...
            for spn_obj_index in range(num_of_shapenet_objects):
                ...
                max_r = np.sqrt(
                    np.power(xyz[(labels > 0).squeeze(1)][:, 0], 2) + np.power(xyz[(labels > 0).squeeze(1)][:, 1],
                                                                               2)).max()
                random_dx = np.random.uniform(low=min_r, high=np.max((max_r * 0.01,min_r + 1e-1)))
                ...
                #!======================================================
                # write_ply(spn_points,'logs/debug/shape3.ply')
                #!======================================================
                ...
                random_resize_factor = np.random.uniform(low=1, high=4)#* 2 还是4, 不太确定.. 
                spn_points = resize_shapenet_object(spn_points, resize_factor=random_resize_factor)
                #!======================================================
                # write_ply(spn_points,'logs/debug/shape4.ply')
                #!======================================================
                ...
                #!======================================================
                # write_ply(spn_points,'logs/debug/shape5.ply')
                #!======================================================
                ...
                #!======================================================
                # write_ply_color(xyz,labels.squeeze(),'logs/debug/scene2.ply')
                #!======================================================

                # change labels
                # labels[introduced_anomaly_indices] = 17
                labels[introduced_anomaly_indices] = 18 + int((random_dx / (max_r - min_r) )*100)

                #!======================================================
                # write_ply_color(xyz,labels.squeeze(),'logs/debug/scene3.ply')
                #!======================================================
...
@register_dataset
class cylinder_dataset_nuscenes_panop_incre(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=0, return_test=False,
                 fixed_volume_space=False, max_volume_space=[50, np.pi, 3], min_volume_space=[0, -np.pi, -5],
                 scale_aug=False, transform_aug=False, trans_std=[0.1, 0.1, 0.1],
                 min_rad=-np.pi / 4, max_rad=np.pi / 4, ds_sample=False, incre=None):
