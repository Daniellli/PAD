...
def main(args):
    ...
    if os.environ.get('LOCAL_RANK') is not None:
        ...
        if int(os.environ['LOCAL_RANK']) == 0:
            experiment_output_dir = join(os.getcwd(),'nuscenes_runs', time.strftime("%Y-%m-%d-%H:%M:%s",time.gmtime(time.time())))
    ...
    writer = SummaryWriter('../nuscenes_runs/' + 'debug')
    while epoch < train_hypers['max_num_epochs']:
        ...
        if False:
            ...
            if gpu_id == 0:
                ...
                save_path_for_current_epoch = model_latest_path[
                                              :model_latest_path.rfind('/')] + '/' + 'model_epoch_' + str(epoch) + '.pt'
                if DISTRIBUTED:
            ...
            mean_val_loss = torch.tensor(mean_val_loss).cuda()
            if DISTRIBUTED:
                torch.distributed.barrier()
                torch.distributed.all_reduce(mean_val_loss)
                mean_val_loss = mean_val_loss.cpu().numpy() / get_world_size()
            else:
                mean_val_loss = mean_val_loss.cpu().numpy()
            if gpu_id == 0:
                print('Current val loss is %.3f' %
                      (mean_val_loss))
        ...
        for i_iter, (_, train_vox_label, train_grid, _, train_pt_fea) in enumerate(train_dataset_loader):
            ...
            if DISTRIBUTED:
                ...
                loss_mean = loss / get_world_size()
            else:
                loss_mean = loss
            if gpu_id == 0:
                loss_list.append(loss_mean.item())

            # if global_iter % 1000 == 0 and gpu_id ==0:
            #     if len(loss_list) > 0:
            ...
            if DISTRIBUTED:
                ...
                pbar.update(1)
            global_iter += 1
            # if global_iter % check_iter == 0 and gpu_id == 0:
            #     if len(loss_list) > 0:
            #         print('epoch %d iter %5d, loss: %.3f\n' %
            #               (epoch, i_iter, np.mean(loss_list)))
            #     else:
