...
def main(args):
    ...
    while epoch < train_hypers['max_num_epochs']:
        ...
        for i_iter, (_, train_vox_label, train_grid, _, train_pt_fea, dis_labels) in enumerate(train_dataset_loader):
            ...
            if unknown_cls_index not in torch.unique(point_label_tensor):
                pbar.update(1)
                global_iter += 1
            ...
            voxel_label_origin_new[voxel_label_origin_new == 17] = 0
            ...
            label_dummy = torch.ones(output_normal_dummy_2.shape[0]).type(torch.LongTensor).cuda() * 17
