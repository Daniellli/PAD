...
'''
description:  given the anomaly point with special color
param {*} points
param {*} labels
param {*} filename
param {*} num_classes
...
return {*}
'''
def write_ply_color_anomaly(points, labels, filename, num_classes=None, colormap=pyplot.cm.jet):
    """ Color (N,3) points with labels (N) within range 0 ~ num_classes-1 as OBJ file """
    labels = labels.astype(int)
    N = points.shape[0]
    if num_classes is None:
...
def write_oriented_bbox(scene_bbox, out_filename, colors=None):
    ...
    def convert_oriented_box_to_trimesh_fmt(box, color=None):
        ...
        if color is not None:
            box_trimesh_fmt.visual.vertex_colors=color
        return box_trimesh_fmt
    ...
    for i, box in enumerate(scene_bbox):
        ...
        if colors is not None:
            color = 255*np.ones((8,4)).astype(np.uint8)
            color[:,:3] = colors[i].reshape(1,3)
        scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box, color))        
...
""""
coordinate_list: is the voxel grid 

color_list : the labels

"""
def plot_3d_points_by_group(coordinate_list, color_list, 
                            index=0, angle_x=0, angle_y=0,
                            color_mapping=None,
                            save_name='logs/debug/tmp.png',index_to_name=None):
    
    assert color_mapping is not None  and index_to_name is not None
